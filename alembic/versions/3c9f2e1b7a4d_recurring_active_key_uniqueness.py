...
def upgrade() -> None:
    """
    Ensure at most one active recurring per merchant and per category, and create partial unique indexes enforcing that.
    
    Deactivate any active recurrings that are duplicates within the same merchant (merchant_id set, category_id NULL) or within the same category (category_id set, merchant_id NULL), keeping the earliest row for each group. Then create two partial unique indexes to enforce a single active recurring per merchant and per category: ux_recurrings_active_merchant_id (on merchant_id) and ux_recurrings_active_category_id (on category_id).
    """
    op.execute(
        """
        WITH ranked AS (
...
def downgrade() -> None:
    """
    Remove the partial unique indexes enforcing one active recurring per merchant or category.
    
    This drops the ux_recurrings_active_category_id and ux_recurrings_active_merchant_id indexes on the recurrings table.
    """
