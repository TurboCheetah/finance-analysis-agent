...
def upgrade() -> None:
    """
    Apply schema changes that enforce a single active recurring-missed review item per referenced recurring event.
    
    This migration marks any duplicate active recurring-missed review_items (same ref_table/ref_id/item_type/source) as resolved and sets their resolved_at when missing, then creates a unique partial index to prevent future duplicates for active recurring-missed items.
    """
    op.execute(
        """
        WITH ranked AS (
...
def downgrade() -> None:
    """
    Remove the uniqueness guard for active recurring missed review items.
    
    This downgrades the schema change performed in the upgrade by dropping the
    partial unique index named "ux_review_items_active_recurring_missed_event"
    from the "review_items" table.
    """
