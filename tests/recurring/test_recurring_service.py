...
def _seed_account(session: Session, *, account_id: str = "acct-1") -> None:
    """
    Add a simple Account entity with the given id to the provided DB session.
    
    Parameters:
        account_id (str): Identifier to assign to the created Account (default "acct-1").
    """
...
def _seed_merchant(session: Session, *, merchant_id: str = "mer-1", name: str = "Gym") -> None:
    """
    Insert a Merchant row into the given database session for test setup.
    
    Parameters:
        session (Session): Database session to add the Merchant to.
        merchant_id (str): Identifier to assign to the new Merchant (default "mer-1").
        name (str): Canonical name to assign to the new Merchant (default "Gym").
    """
...
def _seed_transaction(
    ...
    *,
    ...
    """
    Create and add a Transaction record to the given database session with sensible defaults for testing.
    
    Parameters:
        session (Session): Database session to which the Transaction will be added.
        transaction_id (str): Unique identifier to assign to the Transaction.
        posted_date (date): Posted and effective date for the Transaction.
        merchant_id (str | None): Merchant identifier to associate with the Transaction, or None.
        category_id (str | None): Category identifier to associate with the Transaction, or None.
        amount (str): Decimal amount as a string (e.g., "-25.00"); converted to Decimal before storing.
    """
    now = utcnow()
...
def test_recurring_detect_resolves_missed_review_items_when_event_becomes_observed(db_session: Session) -> None:
    """
    Verifies that missed recurring-event review items are resolved when corresponding events become observed.
    
    Seeds an account, merchant, and initial transactions to produce missed recurring events with review items, then adds matching transactions and re-runs detection; asserts there are zero active missed-event review items and two resolved review items.
    """
    _seed_account(db_session)
    _seed_merchant(db_session, merchant_id="mer-gym", name="Gym")
    _seed_transaction(db_session, transaction_id="txn-1", posted_date=date(2026, 1, 1), merchant_id="mer-gym")
...
def test_recurring_detect_raises_when_expected_date_generation_exceeds_limit(db_session: Session) -> None:
    """
    Verifies that recurring detection raises an error when generating expected event dates would exceed the configured iteration limit.
    
    Seeds an account, a merchant, and three weekly transactions, then calls recurring_detect_and_schedule with max_expected_iterations set below the required number of generated expected dates and asserts a ValueError with message "max_expected_iterations exceeded".
    """
    _seed_account(db_session)
    _seed_merchant(db_session, merchant_id="mer-gym", name="Gym")
    _seed_transaction(db_session, transaction_id="txn-1", posted_date=date(2026, 1, 1), merchant_id="mer-gym")
