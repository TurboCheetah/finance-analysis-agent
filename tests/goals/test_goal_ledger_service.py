...
def _seed_account(session: Session, *, account_id: str = "acct-1") -> None:
    """
    Create and add a default checking Account to the provided database session.
    
    Parameters:
        account_id (str): Identifier to assign to the created Account (default "acct-1"). The Account is added to the given session.
    """
...
def _seed_goal(
    ...
    *,
    ...
    """
    Create and add a Goal record to the given database session using the provided attributes.
    
    Parameters:
    	goal_id (str): Identifier to assign to the Goal record.
    	target_amount (str): Decimal string for the goal target amount (e.g., "1000.00"); converted to Decimal.
    	target_date (date | None): Target completion date for the goal, or None.
    	monthly_contribution (str | None): Decimal string for monthly contribution (e.g., "50.00") or None; converted to Decimal when provided.
    	spending_reduces_progress (bool): If True, spending is considered to reduce the goal's progress.
    	status (str): Goal status value to store (e.g., "active").
    """
    session.add(
        Goal(
            id=goal_id,
...
def _seed_transaction(
    ...
    *,
    ...
    """
    Create and add a Transaction record to the provided database session.
    
    Parameters:
        transaction_id (str): Identifier to assign to the new Transaction.
        posted_date (date): Date to use for both the Transaction's posted_date and effective_date.
        amount (str): Decimal string used for both amount and original_amount (converted to Decimal).
    
    Notes:
        The created Transaction is associated with account "acct-1", uses USD as currency, has pending_status "posted",
        source_kind "manual" and source_transaction_id derived from the transaction_id. created_at and updated_at are set to the current UTC time.
    """
    now = utcnow()
...
def _seed_goal_event(
    ...
    *,
    ...
    """
    Create and add a GoalEvent that links a goal to a (optional) transaction.
    
    Parameters:
        event_id (str): Identifier for the GoalEvent to create.
        goal_id (str): Identifier of the goal this event applies to.
        event_date (date): Date of the event.
        amount (str): Decimal amount as a string; converted to Decimal for the event.
        related_transaction_id (str | None): Optional identifier of the related transaction.
    
    Notes:
        The created GoalEvent will have event_type "rule.linked_transaction" and metadata_json set to None.
    """
    session.add(
        GoalEvent(
            id=event_id,
