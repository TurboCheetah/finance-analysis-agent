...
def test_recurring_missed_review_item_uniqueness_migration_dedupes_active_rows(tmp_path: Path) -> None:
    """
    Verify that the migration deduplicating active `recurring_missed_event` review items resolves older duplicates and enforces a uniqueness constraint.
    
    Sets up a pre-migration state with two active review_items for the same ref_table/ref_id/item_type/source (one older, one newer), runs the migration, and asserts that the older item is marked `resolved` (with `resolved_at` set), the newer remains `in_progress`, only one active item exists, and inserting another conflicting active item afterwards raises an IntegrityError.
    """
