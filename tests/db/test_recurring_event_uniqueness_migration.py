...
def test_recurring_event_uniqueness_migration_dedupes_and_remaps_review_refs(tmp_path: Path) -> None:
    """
    Verify that the migration deduplicates duplicate recurring_events for the same recurring and expected date and remaps any review_items referencing the removed event to the surviving event.
    
    Sets up a pre-migration database state with two recurring_events for the same recurring_id and expected_date and a review_item referencing the to-be-removed event, runs the migration to head, and asserts that only the surviving recurring_event remains, the review_item's ref_id is updated to the surviving event, and that the database enforces uniqueness by rejecting a new conflicting insert.
    """
